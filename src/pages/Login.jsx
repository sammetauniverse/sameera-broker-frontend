...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // (If backend sends HTML error instead of JSON, this prevents crash)
      let data;
      try {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-600 via-purple-500 to-blue-500 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md">
        ...
        {/* FORM TAG HANDLES "ENTER" KEY SUBMISSION CORRECTLY */}
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-purple-600 text-white py-3 rounded-lg font-semibold hover:bg-purple-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex justify-center"
          >
            {loading ? (
