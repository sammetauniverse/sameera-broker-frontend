...
export default function AddLeadModal({ isOpen, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // --- BUG FIX: Convert empty string to null ---
      // Django DB rejects empty strings "" for URLs. It needs null.
      const payload = {
        ...formData,
      ...
      // onClose is handled by parent if successful
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors ${uploadSuccess ? 'border-green-400 bg-green-50' : 'border-gray-300 bg-gray-50'}`}>
            ...
            <label htmlFor="modal-upload" className={`cursor-pointer flex flex-col items-center ${uploading ? 'opacity-50' : ''}`}>
              {uploading ? <Loader className="animate-spin text-indigo-600 w-8 h-8 mb-2"/> : 
               uploadSuccess ? <FileText className="text-green-600 w-8 h-8 mb-2"/> : 
               <Upload className="text-gray-400 w-8 h-8 mb-2"/>}
              <span className="text-sm font-medium text-gray-700">
                {uploading ? "Uploading..." : uploadSuccess ? "Document Attached!" : "Click to Upload Document"}
              </span>
            </label>
          </div>
