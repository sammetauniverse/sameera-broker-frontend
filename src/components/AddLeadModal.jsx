...
export default function AddLeadModal({ isOpen, onClose, onLeadSave, initialData }) { // <-- RENAMED PROP
  ...
  const handleSubmit = (e) => {
    ...
    if (typeof onLeadSave === 'function') { // <-- USE RENAMED PROP
      onLeadSave({
        ...formData,
        files: files
      });
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-4">
          ...
          <label><input type="checkbox" checked={formData.site_visit_done} onChange={e => setFormData({ ...formData, site_visit_done: e.target.checked })} /> Site Visit Completed?</label>
          <label><input type="checkbox" checked={formData.is_converted} onChange={e => setFormData({ ...formData, is_converted: e.target.checked })} /> Converted?</label>
